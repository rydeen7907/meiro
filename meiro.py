    ...
    def update(self):
        ...
        if self.walking == 0:
            ...
            if press[pygame.K_h]:
                ...
                question_1 = \
                    font1.render("Do you use your special ability?", True, BLUE)
                screen.blit(question_1, (50, 150))
                answer_1 = \
                    font1.render("A : YES", True, BLUE)
                screen.blit(answer_1, (280, 200))
                # answer_2 = \
                    # font1.render("n :  No ", True, BLUE)
        ...
        ##                            pygame.draw.rect(screen, (125,125,0), pygame.Rect(270,245,150,45), 2)
            ...
            if hukidashi != 0:
                commentimage = pygame.image.load("comment1.png").convert()
        ...
        if len(hitlist) >= 2:
            for hit in hitlist:
                if hit == player:
                    ...
                    imagetext1 = font1.render("GOAL!", True, BLUE)
                    screen.blit(imagetext1, (260, 150))
                    btn1image = pygame.image.load("retry1.png").convert()
                    rect_btn1image = btn1image.get_rect().move(180, 200)
                    screen.blit(btn1image, rect_btn1image)
                    imagetext2 = font1.render("or", True, BLUE)
                    screen.blit(imagetext2, (300, 215))
                    btn2image = pygame.image.load("quit1.png").convert()
...
# 迷路のブロックは乱数(0-100)により生成する(調整可能).8行38列.
...
blockimage = \
    pygame.image.load("grass.png").convert()
...
goal = Goal(size * 38, size * 8, "korilakkuma.png")
allgroup.add(goal)
player = Player(size * 1, size * 1, "rilakkuma.png")
enemys = [i for i in range(15)] # range(15)で固定
for i in range(15):
    enemys[i] = Enemy(size * ((i + 1) * 2), size * (i * 0.5), "ken_cg.png")
